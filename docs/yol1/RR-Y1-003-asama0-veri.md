...
| `TP.TCMB.PFAIZ` | (spec-adayi, politika faizi) | ? | **UNVERIFIED** -- katalogda YOK, HIC test-edilmedi | -- | -- |
| `TP.FAIZ.PYUVDL` | eski TLREF kodu | -- | **DEAD** (HTTP 400) | -- | -- |

- **`TP.TCMB.PFAIZ`** (specin birincil-adayi) yalniz `src/data/evds_client.py` docstring-
...
Spec bu serinin varligini "BELIRSIZ (kod-bilinmiyor)" saymisti. **BULUNDU + KANIT VAR:**
...
| 1b | Spec-adayi `TP.TCMB.PFAIZ` calisiyor mu? | **TEYITSIZ** (katalogda yok, hic test-edilmedi). Redundant -- onaylanmis alternatif var. |
...
| Nominal-faiz EVDS ~50/50 | NETLESTI: spec-adayi PFAIZ teyitsiz AMA 2 alternatif (APIFON4, BISTTLREF.ORAN) CONFIRMED-ACTIVE -> nominal-bacak fizibil. |
